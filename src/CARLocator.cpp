...
cv::Rect PhCARLocator::getRMLocation(Blobs& blobs, cv::Rect& carLoc) {
	if (boundingBox) {
		return CARLocator::getRMLocation(blobs, carLoc);
	}
...
cv::Rect PhCARLocator::getCARLocation() {
	...
	int x = 0.7 * cheqImg.cols;
	int y = 0.2 * cheqImg.rows;
	Rect roi(x, y, cheqImg.cols - x, cheqImg.rows * 0.2);
	Mat img = this->cheqImg(roi);
	Mat edges;
	Canny(img, edges, 80, 120);
	std::vector<cv::Vec4i> lines;
	...
	roi.width = maxX - minX + 1;
	roi.height = maxY - minY + 1;
	cv::rectangle(cdst, roi, CV_RGB(0,0,0));
	imshow("car", cdst);
	return roi;
}
...
void PhCARLocator::getHandwrittenBlobs(Blobs& blobs) {
	Mat bin;
	GaussianBlur(cheqImg, bin,Size(3,3),0);
	threshold(bin,bin,128,255,CV_THRESH_OTSU | CV_THRESH_BINARY_INV);
	bin = bin / 255;
	findBlobs(bin, blobs);
	Blobs rmBlobs;
	for (size_t i = 0; i < blobs.size(); ++i) {
