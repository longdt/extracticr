...
void loadBCGTrainData(std::vector<vec_t>& inputs,
	...
	for (auto it = segmStrs.begin(), end = segmStrs.end(); it != end; ++it) {
		...
		nr.genTrainData(it->second, ICR_BCG, inputs, labels);
	}
}

int bcgmain(int argc, char **argv) {
	typedef network<mse, gradient_descent> MLP;
	MLP mynet;
	fully_connected_layer<MLP, sigmoid_activation> F1(72, 120);
	fully_connected_layer<MLP, sigmoid_activation> F2(120, 64);
	mynet.add(&F1);
	mynet.add(&F2);

	std::vector<label_t> train_labels;
	std::vector<vec_t> train_inputs;
	loadBCGTrainData(train_inputs, train_labels);
	std::ifstream ifs("bcg");
	if (ifs.is_open()) {
		ifs >> F1 >> F2;
		ifs.close();
	}
